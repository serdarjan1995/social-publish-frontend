  ...
  <div class="card-form">
    ...
    <div class="card-form__inner">
      <div class="card-input">
        <label for="cardNumber" class="card-input__label">{{ $t('Card.cardNumber') }}</label>
        <input
            type="tel"
      ...
      <div class="card-input">
        <label for="cardName" class="card-input__label">{{ $t('Card.cardName') }}</label>
        <input
            type="text"
            :id="fields.cardName"
            v-letter-only
      ...
      <div class="card-form__row">
        <div class="card-form__col">
          <div class="card-form__group">
            <label for="cardMonth" class="card-input__label">{{ $t('Card.expirationDate') }}</label>
            <select
                ...
                data-card-field
            >
              <option value disabled selected>{{ $t('Card.month') }}</option>
              <option
            ...
            <select
                ...
                data-card-field
            >
              <option value disabled selected>{{ $t('Card.year') }}</option>
              <option
        ...
        <div class="card-form__col -cvv">
          <div class="card-input">
            <label for="cardCvv" class="card-input__label">{{ $t('Card.CVV') }}</label>
            <input
                type="tel"
                class="card-input__input"
                v-number-only
      ...
      <button class="card-form__button" v-on:click="invaildCard">{{ $t('Card.submit') }}</button>
    </div>
  </div>
</template>

<script>
import Card from "@/views/SocialMedia/Admin/Payment/Component/Card";
export default {
  name: 'CardForm',
  directives: {
    'number-only': {
  ...
  methods: {
    ...
    invaildCard () {
      ...
      if (sum % 10 !== 0) {
        const obj = {
          plan_id: 1,
          card_no: this.formData.cardNumber,
          card_expiry: this.formData.cardMonth+'.'+this.formData.cardYear,
          card_cvv: this.formData.cardCvv,
          card_name: this.formData.cardName
        };
        this.$store.dispatch("payments/createPayment", obj)
      }
      else{
        const obj = {
          plan_id: 1,
